...
class CMT(nn.Module):
    def __init__(self, in_channel, img_size, num_head, split_size, depth):
        super().__init__()
        self.tran_block = nn.ModuleList([CSWinBlock(
            dim=in_channel, num_heads=num_head, reso=img_size, mlp_ratio=4.,
            qkv_bias=True, qk_scale=None, split_size=split_size,
            drop=0., attn_drop=0.,
            drop_path=0, norm_layer=nn.LayerNorm) for i in range(depth)])
    ...
    def forward(self, cnn_input, trans_input):
        ...
        # print('cnn',cnn_x.shape, 'trans', trans_input.shape)
        # trans_new = torch.cat((trans_input, cnn_x), dim=2)  # B, L, 2C
        trans_out = trans_input + cnn_x
        for block in self.tran_block:
            trans_out = block(trans_out)
        # result = self.down_dim(trans_out)
        result = self.norm(trans_out)
        # print('trans:',trans_out.shape)
        # result = self.patch_expand(trans_out)
...
class CSWinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2, 2, 6, 2],
        ...
        #################CMT##################
        self.cmt_1 = CMT(self.embed_dim, img_size // 4, self.s_head[0], self.s_size[0], depth[0])
        self.cmt_2 = CMT(self.embed_dim * 2, img_size // 8, self.s_head[1], self.s_size[1], depth[0])
        self.cmt_3 = CMT(self.embed_dim * 4, img_size // 16, self.s_head[2], self.s_size[2], depth[0])
    ...
    def cnn_sup(self, inputs, x):
        ...
        up_1_1 = torch.cat((down_2, up_1), dim=1)
        up_2 = self.cnn_up(up_1_1)
        up_2_2 = torch.cat((up_2, down_1), dim=1)
        return up_2_2
