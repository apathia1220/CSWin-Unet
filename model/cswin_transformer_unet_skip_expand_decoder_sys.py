...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape
        H = W = int(np.sqrt(L))
        # print('x_ex_1:', x.shape)
        x = self.expand(x)
        # print('x_ex_2:', x.shape)
        C = C * 2
        x = x.view(B, H, W, C)
        # print('x_ex_3:', x.shape)
        x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=C//4)
        # print('x_ex_4:', x.shape)
        x = x.view(B, -1, C//4)
        # print('x_ex_5:',x.shape)
        x = self.norm(x)
        # print('x_ex_6:', x.shape)
...
class Downsample_gt(nn.Module):
    def __init__(self, dim, num_classes):
        super().__init__()
        self.conv_1 = nn.Conv2d(dim, num_classes, kernel_size=2, stride=2)
...
class CNN_Downsample(nn.Module):
    ...
    """
    def __init__(self, in_ch, embed_dim=16):
        super().__init__()
        filters = [embed_dim, embed_dim*2, embed_dim*4, embed_dim*8, embed_dim*16]
...
class CMT(nn.Module):
    ...
    def forward(self, cnn_input, trans_input):
        ...
        cnn_x = self.ap(cnn_input)   # cnn_x: B, C, 1, 1
        # print('cnn_x:',cnn_x.shape)
        cnn_x = cnn_x.view(B, C, -1)   # B, C, 1
        cnn_x = cnn_x.permute(0, 2, 1)  # B, 1, C
        trans_new = torch.cat((trans_input, cnn_x), dim=1)  # B, L+1, C
        trans_out = self.tran_block(trans_new)
        result = torch.split(trans_out, L, dim=1)  # B, L, C
        # result = result.view()
...
class CSWinTransformerSys(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2, 2, 6, 2],depths_decoder=[2, 2, 6 ,2],
                 split_size=[3, 5, 7],
        ...
        #################CMT##################
        # self.cmt_1 = CMT(self.embed_dim)
        # self.cmt_2 = CMT(self.embed_dim*2)
        # self.cmt_3 = CMT(self.embed_dim*4)


        ##############conv_supervision##############
        self.cnn_down_1 = conv_block(3, embed_dim)
        self.cnn_down_2 = nn.Sequential(
            conv_block(embed_dim, embed_dim),
            nn.MaxPool2d(kernel_size=2, stride=2)
        )
        self.cnn_up_1 = up_conv(embed_dim, embed_dim)
        self.cnn_up = up_conv(embed_dim , embed_dim)
        curr_dim = embed_dim
        ...
        # self.final.append(nn.Conv2d(in_channels=curr_dim*4, out_channels=self.num_classes, kernel_size=1))
        # self.final.append(nn.Conv2d(in_channels=curr_dim*2, out_channels=self.num_classes, kernel_size=1))
        # self.final.append(nn.Conv2d(in_channels=curr_dim*8, out_channels=self.num_classes, kernel_size=1))
    ...
    def forward_up_feature(self, x, x_downsample):
        ...
        for pre, blocks in zip([self.expand3, self.expand2, self.expand1],
            ...
            # print(x.shape)
            x = pre(x)
            # print(x.shape, x_downsample[2].shape)
            # print('\n')
            # print('x:',x.shape,'\n','x_downsample:',x_downsample[2-idx].shape)
            # B, H, W, C = x.shape
            # x_downsample[2-idx] = x_downsample[2-idx].reshape(B, H, W, C)
            x = torch.cat([x, x_downsample[2-idx]], -1)
    ...
    def cnn_sup(self,inputs, x):
        # print(x.shape)
        B, L, C = x.shape
        H = W = int(np.sqrt(L))
        x = x.view(B, H, W, -1)
        x = x.permute(0, 3, 1, 2)
        down_1 = self.cnn_down_1(inputs)
        # print('down_1:',down_1.shape)
        down_2 = self.cnn_down_2(down_1)
        # print('down_2:',down_2.shape)
        # print('x:',x.shape)
        up_1 = self.cnn_up_1(x)
        up_1_1 = down_2 + up_1
        up_2 = self.cnn_up(up_1_1)
        up_2_2 = up_2 + down_1
        return up_2_2

        
    # def up_x2(self, samples):
    #     sample_x = []
    #     sample_t = [samples[0], samples[1], samples[3]]
    #     # print('upsample[0].shape',samples[0].shape, 'upsample[1].shape',samples[1].shape, 'upsample[2].shape',samples[3].shape)
    #     for sample, up in zip(sample_t, self.final):
    #         B, L, C = sample.shape
    #         H = W = int(np.sqrt(L))
    #         # print('B:',B,'C:',C,'H:',H,'W:',W)
    #         sample = sample.view(B, H, W, -1)
    #         sample = sample.permute(0, 3, 1, 2)
    #         sample = up(sample)
    #         sample_x.append(sample)
    #     upsample = [sample_x[1], sample_x[0], sample_x[2]]
    #     # print('upsample[0].shape',upsample[0].shape, 'upsample[1].shape',upsample[1].shape, 'upsample[2].shape',upsample[2].shape)
    #     return upsample
    ...
    def forward(self, x):
        ...
        # for item in x_downsample:
        #     print('item:',item.shape)
        # print('e1:', x_e1.shape, 'e2:', x_e2.shape,'e3:', x_e3.shape,)
        # x_temp = x
        # downsample_cmt = []
        # downsample_cmt.append(self.cmt_1(x_e1, x_downsample[0])[0])
        # downsample_cmt.append(self.cmt_2(x_e2, x_downsample[1])[0])
        # downsample_cmt.append(self.cmt_3(x_e3, x_downsample[2])[0])
        # for i in downsample_cmt:
        #     print(i.shape)
        x = self.forward_up_feature(x, x_downsample)
        # x, upsample_x = self.forward_up_feature(x, x_downsample)
        # print('temp:',x_temp.shape)
        # upsample_x.append(x_temp)
        # print('good:',upsample_x[3].shape)
        # upsample = self.up_x2(upsample_x)
        x = self.up_x4(x)
        print("x:",x.shape, "input:", inputs.shape)
        x = self.cnn_sup(inputs, x)
        x = self.output(x)
        return x